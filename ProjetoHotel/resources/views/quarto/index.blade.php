...
@section('content')
    ...
    <div class="container">
        <div class="app-title">
            <h1>
                <i class="fa fa-edit">Lista de Quarto</i>
            </h1>
    ...
    <div class="container">
        <div class="tile">
            <div class="tile-body">
                <form class="form form-inline" action="{{ url('search_quarto') }}" method="POST">
                    @csrf
                    <div class="col-sm-12">
...
{{-- 
                            ...
                            <td data-title="Telefone"  style="text-align: center" >{{$registro->telefone }}</td>
                            <td data-title="Ações"  style="text-align: center"></td>
                            <td>
                                <a class="btn btn-info btn-sm" href="{{ url('/quarto/alterar', $registro->id) }}"><i class="fa fa-pencil"></i></a>
                                <a class="btn btn-danger btn-sm" href="{{ url('/quarto/excluir', $registro->id) }}"><i class="fa fa-trash"></i></a>
                                <a class="btn btn-warning btn-sm" href="{{ url('/quarto/consultar', $registro->id) }}"><i class="fa fa-address-book"></i></a>
                            </td>
                            </tr>
                            @endforeach  ERRO NO quarto/LISTAR--}}
                        </tbody>
                    </table>
                   {{--  @if(@isset($filters))
                    ...
                    @else
                        {{$registros->links()}}  
                    @endisset ERRO NO quarto/LISTAR--}}
                    <a class="btn btn-success btn-lg" href="{{ url('/quarto/cadastrar')}}"><i class="fa fa-circle"></i></a>
                </div>
            </div>
        </div>
    </div>
